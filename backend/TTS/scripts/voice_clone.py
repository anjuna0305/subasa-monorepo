...
def generate_tts_output(input_text, output_dir, output_filename, speaker_wav=None):
    """
    Generate TTS output with voice cloning if speaker_wav is provided.
    """
    # Preprocess the input text
    preprocessed_text = preprocess_text(input_text)
    ...
    os.makedirs(output_dir, exist_ok=True)
    audio_path = os.path.join(output_dir, output_filename)
    tts.tts_to_file(text=preprocessed_text, speaker_wav=speaker_wav, file_path=audio_path)
    print(f"Generated audio saved to: {audio_path}")
...
def generate_audios_from_file(file_path, output_dir="test_audios", base_filename="met_sin"):
    ...
    """
    sentence = "සිරිකොත ඉදිරිපිට ගැටුමක්"

    # Generate a filename like "met_sin_1.wav", "met_sin_2.wav", etc.
    output_filename = f"{base_filename}.wav"
    speaker_wav = r"voice_clone/ref_voice1.wav"
    encoder = VoiceEncoder()
    wav, _ = librosa.load(speaker_wav, sr=22050)  # Load audio at 22050 Hz
    wav = preprocess_wav(wav)
    _speaker_embedding = encoder.embed_utterance(wav)
    generate_tts_output(sentence, output_dir, output_filename,speaker_wav)
